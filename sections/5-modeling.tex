...
我们以整个装置静止直立在水面上时，浮子的质心为原点，以来流方向为 $x$ 轴正方向，以铅直向上为 $z$ 轴正方向，建立空间直角坐标系 $O-xyz$。
...
\begin{align}
    m\cdot\dfrac{\dif^2 x_P}{\dif t^2} &= F_\text{PTO}\sin(\theta+\gamma)+F_{ABx} \\
    m\cdot\dfrac{\dif^2 z_P}{\dif t^2} &= F_\text{PTO}\cos(\theta+\gamma)-mg+F_{ABz}
\end{align}

考虑振子的转动，设其在弹簧长度为 $l$ 时的转动惯量为 $I_B(l)$。
记绞接处旋转阻尼器产生的旋转力矩为 $M_{PTO}$，则可以列出如下的转动运动学方程
\begin{equation}
    I_B(l)\cdot\dfrac{\dif^2(\theta+\gamma)}{\dif t^2}=M_{PTO}-F_{ABx}\cdot l\cos(\alpha+\gamma)-F_{ABz}\cdot l\sin(\theta+\gamma)
\end{equation}